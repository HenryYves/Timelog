...
### 14. background-size + repeat 平铺被拉伸，导出模糊

水印平铺最初用 canvas 瓦片 + CSS `background: url(tile) repeat` + `background-size` 实现。预览正常，但 html2canvas（scale=2）导出时对 `background-size` + `repeat` 组合的处理有坑：瓦片被拉伸，整张导出图里只有平铺层糊。

**方案**：不要走 CSS 背景平铺。生成一张与导出区域同尺寸（×scale）的完整 canvas，JS 循环绘制所有平铺单元（错排：`buildWatermarkOverlay()`），`toDataURL` 后以 `<img>` 绝对定位覆盖。`<img>` 是 html2canvas 最稳定的渲染路径。见 `frontend/src/components/ExportImagePanel.vue`。

### 15. linear-gradient 中 `transparent` 关键字 html2canvas 渲染为灰色
