...
- Restore button SVG at `frontend/public/icons/恢复.svg`
...
**Files:**
- Copy: `D:\a_my\seldom\computer_system\transfer\SP_xx_zd\恢复.svg` → `frontend/public/icons/恢复.svg`
...
```bash
cp "D:\a_my\seldom\computer_system\transfer\SP_xx_zd\恢复.svg" "D:\a_my\project\html\Timelog\frontend\public\icons\恢复.svg"
```
...
```bash
git add frontend/public/icons/恢复.svg frontend/src/constants.js frontend/src/store/settings.js frontend/src/strings.js
...
Template changes: replace `<row>` → `<div class="row">` and `<btn-restore>` → `<button class="btn-restore" title="恢复默认设置"><img src="/icons/恢复.svg" alt=""></button>`.
