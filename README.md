...
- [x] ↑↓ 键微调时间块时间范围无实际效果
- [x] `More` 菜单打开时未拦截 <kbd>t</kbd> 快捷键
- [x] `More` 菜单应在任意操作后自动关闭

### Feature

- [x] 自动更新：可关闭、更新前提醒、手动检测、多 URL 回退
- [x] 使用<kbd>n</kbd>可以使用格式化文字快速创建时间块
- [ ] <kbd>t</kbd> /<kbd>n</kbd>创建时间块智能默认时间（当天最后时间块结束 → 当前时间）
- [x] 编辑框取消按钮弹窗确认
- [ ] 列表自动续集 + 有序列表自动重排序
- [ ] 设置项：「保存」按钮出现在「取消」左边
- [ ] 自定义字体大小 / 字体
