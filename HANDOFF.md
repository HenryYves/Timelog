...
- 时间块透明度可调（>100% 时色彩增强）
- 自动清理旧数据（设置保留最近 N 个有数据的天）
- 数据管理：日期段删除、全部删除
- 自定义 alert/confirm 弹窗（居中、焦点陷阱、ESC 按层级关闭）
...
```
b5ca8c0 chore: 版本号 0.2.14 → 0.3.0
4c801de feat(dialog): 自定义 alert/confirm 弹窗替代浏览器原生对话框
...
│   ├── tauri.conf.json     ← Tauri 配置（withGlobalTauri: true）
